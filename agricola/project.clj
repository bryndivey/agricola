(defproject agricola "0.0.1-SNAPSHOT"
  ...
  :aliases {"run-dev" ["trampoline" "run" "-m" "agricola.server/run-dev"]}
  :repl-options  {:init-ns user
                  :init (try
                          (use 'io.pedestal.service-tools.dev)
                          (require 'agricola.service)
                          ;; Nasty trick to get around being unable to reference non-clojure.core symbols in :init
                          (eval '(init agricola.service/service #'agricola.service/routes))
                          (catch Throwable t
  ...
  :main ^{:skip-aot true} agricola.server)
